...
function BrandForm({ value, brand, updateMyBrand }) {
  //   const { setGetProduct, getProduct } = useContext(AuthContext);
  const url = brand
    ? `http://localhost:3000/brands//updateBrand/${brand._id}`
    : "http://localhost:3000/brands/addBrand";

  const method = brand ? "put" : "post";
  const formData = async (e) => {
    e.preventDefault();
    try {
      const brandForm = new FormData(e.target);
      updateMyBrand(brandForm, url, method);
